...
<body>
  ...
  <main class="container py-4">
    ...
    <form id="filterForm" class="row g-2 align-items-end mb-4" method="get" action="${listAction}">
      <div class="col-sm-4 col-md-3">
        <label class="form-label">카테고리</label>
        <select class="form-select" name="categoryId">
          <option value="">전체</option>
          <c:forEach var="cvo" items="${category}">
            <option value="${cvo.categoryId}" <c:if test="${categoryId == cvo.categoryId}">selected</c:if>>
      ...
      <div class="col-sm-4 col-md-3">
        ...
        <select class="form-select" name="sort">
          ...
          <option value="deadline" <c:if test="${sort=='deadline'}">selected</c:if>>마감 임박순</option>
        </select>
      </div>

      <div class="col-sm-4 col-md-3">
        <div class="form-check mt-4 pt-2">
          <input type="hidden" name="onlyRecruiting" value="false" id="onlyRecruitingHidden">
          <input class="form-check-input" type="checkbox" id="onlyRecruiting"
                 name="onlyRecruiting" value="true" <c:if test="${onlyRecruiting}">checked</c:if>>
          <label class="form-check-label" for="onlyRecruiting">모집 중만 보기</label>
        </div>
      </div>
    ...
    <div class="d-flex justify-content-between align-items-center mb-3">
      <div class="text-muted">총 <strong>${dataCount}</strong>개</div>
    </div>
          ...
          <c:forEach var="w" items="${list}">
            <div class="col-12 col-sm-6 col-lg-4">
              ...
              <c:choose>
                ...
                <c:when test="${fn:startsWith(raw, 'http://') or fn:startsWith(raw, 'https://') or fn:startsWith(raw, '/')}">
              ...
              <a href="${detailUrl}" class="text-decoration-none text-reset">
                <div class="workshop-card">
                  ...
                  <img class="workshop-thumb"
                       src="${thumbUrl}"
                       alt="<c:out value='${w.workshopTitle}'/>"
                  ...
                  <div class="p-3">
                    <c:choose>
                      <c:when test="${w.workshopStatus == 1}">
                        <span class="badge bg-success-subtle text-success badge-status me-1">모집중</span>
                      </c:when>
                      <c:when test="${w.workshopStatus == 0}">
                        <span class="badge bg-secondary-subtle text-secondary badge-status me-1">마감</span>
                      </c:when>
                      <c:when test="${w.workshopStatus == 2}">
                        <span class="badge bg-danger-subtle text-danger badge-status me-1">취소</span>
                      </c:when>
                    </c:choose>

                    <c:if test="${not empty w.categoryName}">
                      <span class="badge bg-light text-dark border"><c:out value="${w.categoryName}" /></span>
                    </c:if>

                    <div class="workshop-title mt-2"><c:out value="${w.workshopTitle}" /></div>
                    <div class="workshop-desc"><c:out value="${w.workshopContent}" /></div>
                      ...
                      <span class="me-2">
                        ...
                        <c:choose>
                          <c:when test="${not empty w.scheduleDate}">
                            <fmt:formatDate value="${w.scheduleDate}" pattern="yyyy.MM.dd" />
                          </c:when>
                          <c:otherwise>-</c:otherwise>
                        </c:choose>
                      </span>
                      <span class="me-2">정원 : <c:out value="${w.capacity}" /></span>
                      <c:if test="${not empty w.applyDeadline}">
                        <span>마감:
                          <fmt:formatDate value="${w.applyDeadline}" pattern="MM.dd" />
                        </span>
                      </c:if>
                    </div>
                  </div>
      ...
      <nav class="mt-4" aria-label="pagination">
        <ul class="pagination justify-content-center">
          ...
          <c:url var="prevUrl" value="/workshop/list">
            ...
            <c:if test="${onlyRecruiting}"><c:param name="onlyRecruiting" value="true" /></c:if>
          </c:url>
