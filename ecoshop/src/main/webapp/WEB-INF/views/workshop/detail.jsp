...
.review-item, .faq-item { border-bottom: 1px solid #e9ecef; padding: 1rem 0; }
</style>
</head>
<body>
...
<main class="container py-4">
  ...
  <c:set var="rawMain" value="${(not empty photoList and fn:length(photoList) > 0) ? photoList[0].workshopImagePath : ''}" />
  <c:choose>
    <c:when test="${empty rawMain}">
      <c:set var="mainImg" value='${pageContext.request.contextPath}/dist/images/noimage.png' />
    </c:when>
    <c:when test="${fn:startsWith(rawMain, 'http://') 
                or fn:startsWith(rawMain, 'https://') 
                or fn:startsWith(rawMain, '/')}">
      <c:set var="mainImg" value='${rawMain}' />
    </c:when>
    <c:otherwise>
      <c:set var="mainImg" value='${pageContext.request.contextPath}/uploads/workshop/${rawMain}' />
    </c:otherwise>
  </c:choose>

  <div class="row g-4 mb-4">
    <div class="col-md-6">
      <img src="${mainImg}" alt="<c:out value='${dto.workshopTitle}'/>" class="workshop-main-img"
           onerror="if(this.src.indexOf('noimage.png')===-1)this.src='${pageContext.request.contextPath}/dist/images/noimage.png'">
    </div>
    <div class="col-md-6">
      <h2><c:out value="${dto.workshopTitle}" /></h2>
      <p class="text-muted mb-2">
        ...
        정원: <c:out value="${dto.capacity}" />명<br>
        마감:
        <c:choose>
  ...
  <ul class="nav nav-tabs" id="workshopTabs" role="tablist">
    <li class="nav-item"><button class="nav-link active" data-bs-toggle="tab" data-bs-target="#intro" type="button">워크샵 소개</button></li>
    <li class="nav-item"><button class="nav-link" data-bs-toggle="tab" data-bs-target="#reviews" type="button">후기</button></li>
  ...
  <div class="tab-content border border-top-0 p-3">
      ...
      <div class="row g-3">
        <c:forEach var="p" items="${photoList}" varStatus="st">
          <c:if test="${st.index > 0}">
            ...
            <c:choose>
              <c:when test="${fn:startsWith(rawPhoto, 'http://') 
                          or fn:startsWith(rawPhoto, 'https://') 
                          or fn:startsWith(rawPhoto, '/')}">
                <c:set var="photoUrl" value='${rawPhoto}' />
              </c:when>
              <c:otherwise>
                <c:set var="photoUrl" value='${pageContext.request.contextPath}/uploads/workshop/${rawPhoto}' />
              </c:otherwise>
            </c:choose>
    ...
    <div class="tab-pane fade" id="reviews">
      <div id="reviewList" class="py-2 text-center text-muted">로딩 중...</div>
    </div>

    <!-- FAQ -->
    <div class="tab-pane fade" id="faq">
      <div id="faqList" class="py-2 text-center text-muted">로딩 중...</div>
    </div>
  </div>
