  ...
  <jsp:include page="/WEB-INF/views/admin/layout/sidebar.jsp" />
		<div class="section p-5" data-aos="fade-up" data-aos-delay="200">
						...
						<div class="right-panel">
						  	<div class="title">
								<h3>${mode=='update'?'공지사항 수정':'공지사항 등록'}</h3>
							</div>
							<hr>
					<div class="board-container row">
						<form name="noticeForm" action="" method="post" enctype="multipart/form-data">
							<table class="table write-form">
								<tr>
									...
									<td>
										<input type="text" name="subject" class="form-control" maxlength="100" placeholder="제목 입력" value="${dto.subject}" required>
									</td>
								</tr>
							...
							<td class="text-start">
								<div class="form-check">
									<input type="checkbox" class="form-check-input" name="notice" id="notice"
									       value="1" ${dto.notice==1 ? "checked":"" }>
									<label for="notice" class="form-check-label ms-1">공지</label>
								</div>
							</td>
						</tr>
						<!--
						<tr>
							...
							<td class="text-start">
								<div class="form-check">
									<input type="checkbox" class="form-check-input" name="showNotice" id="showNotice"
									       value="1" ${mode=="write" || dto.showNotice==1 ? "checked":"" }>
									<label for="showNotice" class="form-check-label ms-1">표시</label>
								</div>
							</td>
						</tr>
  -->
								...
								<tr>
								    ...
								    <td>
								        <textarea name="content" class="form-control" rows="10" placeholder="내용 입력" required>${dto.content}</textarea>
								    </td>
								</tr>
							...
							<div class="text-center">
								<button type="button" class="btn my-btn" onclick="sendOk();">${mode=='update'?'수정완료':'등록완료'}</button>
								<button type="button" class="btn my-btn" onclick="location.href='${pageContext.request.contextPath}/admin/notice/list';">${mode=='update'?'수정취소':'등록취소'}</button>
								<c:if test="${mode=='update'}">
