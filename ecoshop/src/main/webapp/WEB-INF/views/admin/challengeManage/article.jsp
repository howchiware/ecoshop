...
<link rel="stylesheet" href="${cp}/dist/css/main2.css" type="text/css">
<style>
    ...
    <div class="admin-content">
        ...
        <div class="card p-4 challenge-detail">
            ...
            <div class="card-subtitle mb-2 text-muted">
                ...
                <span class="ms-2">ID: ${dto.challengeId}</span>
            </div>
            
            <hr>
            <div class="row g-4">
                <div class="col-lg-7">
                    <p class="mb-4">${dto.description}</p>
                    <c:set var="thumbUrl" value="${empty dto.thumbnail ? cp.concat('/resources/admin/images/add_photo.png') : cp.concat('/uploads/challenge/').concat(dto.thumbnail)}"/>
                ...
                <div class="col-lg-5">
                    <div class="info-grid">
                        <div class="info-card">
                            ...
                            <div class="info-value">${dto.challengeType}</div>
                        </div>
                        <div class="info-card">
                        ...
                        <div class="info-card">
                            ...
                            <div class="info-value">${dto.challengeRegDate}</div>
                        </div>
                        ...
                        <c:if test="${dto.challengeType=='DAILY'}">
                            <div class="info-card">
                                ...
                                <div class="info-value">
                                    <c:choose>
                                        <c:when test="${dto.weekday==0}">일요일</c:when><c:when test="${dto.weekday==1}">월요일</c:when>
                                        <c:when test="${dto.weekday==2}">화요일</c:when><c:when test="${dto.weekday==3}">수요일</c:when>
                                        <c:when test="${dto.weekday==4}">목요일</c:when><c:when test="${dto.weekday==5}">금요일</c:when>
                        ...
                        <c:if test="${dto.challengeType=='SPECIAL'}">
                            <div class="info-card">
                                ...
                                <div class="info-value">${dto.startDate}</div>
                            </div>
                            <div class="info-card">
                                <div class="info-label">종료일</div>
                                <div class="info-value">${dto.endDate}</div>
                            </div>
                            <div class="info-card">
                                <div class="info-label">연속일</div>
                                <div class="info-value">${dto.requireDays}일</div>
                            </div>
                            <div class="info-card">
            ...
            <div class="detail-actions">
                <a href="${cp}/admin/challengeManage/update?challengeId=${dto.challengeId}" class="btn btn-outline-primary">수정</a>
                <!-- 수정된 부분: 삭제 후 현재 페이지로 돌아오도록 page 파라미터 추가 -->
                <a href="${cp}/admin/challengeManage/delete?challengeId=${dto.challengeId}" class="btn btn-outline-danger" onclick="return confirm('정말 삭제하시겠습니까?');">삭제</a>
            </div>
        </div>
    </div>
</div>
