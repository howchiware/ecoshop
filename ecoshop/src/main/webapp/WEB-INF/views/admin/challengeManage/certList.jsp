...
<style>
:root{
  --ink:#334155; --muted-ink:#64748b; --card-bg:#fff; --bd:#e5e7eb;
  --chip-bg:#f8fafc; --chip-bd:#e5e7eb; --chip-ink:#334155;
  ...
  --status-bg:#f3f4f6; --status-bd:#e5e7eb; --status-ink:#344054;
  --dot-pending:#64748b; --dot-approved:#16a34a; --dot-rejected:#e11d48;
}
...
.thumb{ width:88px; height:60px; object-fit:cover; border-radius:10px; border:1px solid #eaeef4; background:#f8fafc; }
...
.pill{ display:inline-flex; align-items:center; justify-content:center; height:28px; padding:0 12px; border-radius:9999px; font-weight:700; letter-spacing:.02em; font-size:12px; }
...
.status-badge{ display:inline-flex; align-items:center; gap:8px; height:24px; padding:0 10px; border-radius:9999px; background:var(--status-bg); border:1px solid var(--status-bd); color:var(--status-ink); font-size:12px; font-weight:700; }
.status-badge::before{ content:""; width:8px; height:8px; border-radius:50%; display:inline-block; background:#cbd5e1; }
.status-0::before{ background:var(--dot-pending); }
.status-1::before{ background:var(--dot-approved); }
.status-2::before{ background:var(--dot-rejected); }
...
.chip-group{ display:flex; align-items:center; gap:6px; background:var(--card-bg); border:1px solid var(--chip-bd); border-radius:9999px; padding:6px; box-shadow:0 1px 2px rgba(0,0,0,.04); }
.chip{ display:inline-flex; align-items:center; gap:8px; padding:6px 10px; border-radius:9999px; font-weight:700; font-size:12px; color:var(--chip-ink); background:var(--chip-bg); border:1px solid var(--chip-bd); }
...
/* hover */
.table.table-hover tbody tr:hover{ background:#f3f4f6 !important; transition: background-color .12s ease; }
.table.table-hover tbody tr:hover .title-link{ text-decoration: underline !important; box-shadow: inset 0 -1px currentColor; }
.cert-table tbody tr:hover{ background:#f3f4f6; transition: background-color .12s ease; }
.cert-table tbody tr:hover .title-link{ box-shadow: inset 0 -1px currentColor; }
.title-link{ display:inline-block; line-height:1.2; }
.table.table-hover tbody tr:hover .title-link, .cert-table tbody tr:hover .title-link{ color:#0f172a; }
</style>
</head>
<body>
  ...
  <div class="admin-content">
    ...
    <form class="row g-2 mb-3" method="get" action="<c:url value='/admin/challengeManage/certList'/>">
      ...
      <div class="col-md-3">
        <select name="approvalStatus" class="form-select">
          ...
          <option value="3" <c:if test="${param.approvalStatus=='2'}">selected</c:if>>자동승인</option>
        </select>
      </div>
      <div class="col-md-2 col-6">
		...
		<c:set var="currentType" value="${not empty type ? type : param.type}" />
		<c:if test="${currentType ne 'DAILY'}">
		  <div class="col-auto ms-auto d-flex align-items-center">
		    <div class="chip-group">
		      <span class="chip pending  ${approvalStatus==0 ? 'is-active' : ''}">
		        <span class="dot"></span> 대기
		        <span class="count">${empty countPending  ? 0 : countPending}</span>
		      </span>
		      <span class="chip approved ${approvalStatus==1 ? 'is-active' : ''}">
		        <span class="dot"></span> 승인
		        <span class="count">${empty countApproved ? 0 : countApproved}</span>
		      </span>
		      <span class="chip rejected ${approvalStatus==2 ? 'is-active' : ''}">
		        <span class="dot"></span> 반려
		        <span class="count">${empty countRejected ? 0 : countRejected}</span>
		      </span>
		    </div>
		  </div>
		</c:if>
    ...
    <table class="cert-table">
      ...
      <c:forEach var="row" items="${list}">
        <tr class="cert-row">
          <!-- 상세 URL -->
          <c:url var="detailUrl" value="/admin/challengeManage/cert/detail">
            <c:param name="postId" value="${row.postId}" />
            ...
            <c:if test="${param.approvalStatus ne null}">
              <c:param name="approvalStatus" value="${param.approvalStatus}" />
            </c:if>
          </c:url>
          ...
          <td>
            <a href="${detailUrl}">
              <c:choose>
                ...
                <c:otherwise><img class="thumb" src="${NOIMG}" /></c:otherwise>
              </c:choose>
            </a>
          </td>


          <td class="col-content position-relative">
          ...
          <td>
            <c:choose>
              ...
              <c:otherwise><span class="pill daily">DAILY</span></c:otherwise>
            </c:choose>
          </td>
          ...
          <td class="col-title">
            <div class="text-truncate">
              <a class="title-link" href="${detailUrl}">
              ...
              <c:when test="${row.challengeType=='SPECIAL'}">
                <div class="btn-line">
                  ...
                  <form method="post" action="<c:url value='/admin/challengeManage/cert/approve'/>">
                    ...
                    <button class="btn-approve" type="submit" ${row.approvalStatus==1 ? 'disabled' : ''}>승인</button>
                  </form>
                  <!-- 반려 -->
                  <form method="post" action="<c:url value='/admin/challengeManage/cert/reject'/>">
                    ...
                    <button class="btn-reject" type="submit" ${row.approvalStatus==2 ? 'disabled' : ''}>반려</button>
                  </form>
                </div>
              </c:when>
              <c:otherwise>
                <div class="text-muted small text-center">자동 처리</div>
              </c:otherwise>
            </c:choose>
          </td>
        </tr>
    ...
    <div class="page-navigation" style="margin-top:16px;">
      ${paging}
    </div>
  </div>
</div>
