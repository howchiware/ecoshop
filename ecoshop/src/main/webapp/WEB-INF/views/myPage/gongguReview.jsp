...
function listReview(page) {
	let url = '${pageContext.request.contextPath}/gongguReview/myReviewlist';
...
function printReview(data) {
	...
	for (const item of list) {
		const { // 구조 분해 할당
			gongguOrderDetailId, rate, content, regDate, answer, answerDate, listReviewImg,
			gongguProductName } = item;
...
$(function(){
	...
	$('.list-review').on('click', '.deleteReview', function(){
		...
		let url = '${pageContext.request.contextPath}/gongguReview/delete';
		let requestParams = 'gongguOrderDetailId=' + gongguOrderDetailId;		
...
function listQuestion(page) {
	let url = '${pageContext.request.contextPath}/gongguInquiry/inquiryList';
	let requstParams = 'pageNo='+page;
...
function printQuestion(data) {
	...
	for (const item of list) { 
		const {
			gongguInquiryId, title, content, regDate, answer, answerDate, gongguProductName,
		} = item;
		...
		out += `
			<div class="mt-1 border-bottom">
				<div class="mt-2 p-2 fw-semibold">\${gongguProductName}</div>
		...
		out += `
			<div class="row p-2">
				...
				<div class="col-auto pt-2 px-0">&nbsp;|&nbsp;<span>\${regDate}</span>
					|<span class="deleteQuestion" data-gongguInquiryId="\${gongguInquiryId}">삭제</span>
				</div>
			`;
...
$(function(){
	$('.list-question').on('click', '.deleteQuestion', function(){
		...
		let url = '${pageContext.request.contextPath}/gongguInquiry/delete';
		let requestParams = 'gongguInquiryId=' + gongguInquiryId;		
