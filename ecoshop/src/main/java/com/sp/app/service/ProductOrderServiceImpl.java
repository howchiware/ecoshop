...
public class ProductOrderServiceImpl implements ProductOrderService {
	...
	public ProductOrder findOrderByOrderDetail(long orderDetailNum) {
		ProductOrder dto = null;
		
		try {
			dto = mapper.findOrderByOrderDetail(orderDetailNum);
		} catch (Exception e) {
	...
	public Payment findPaymentByOrderDetail(Map<String, Object> map) {
		Payment dto = null;
		try {
			dto = Objects.requireNonNull(mapper.findPaymentByOrderDetail(map));
			
			if((dto.getOrderState() == 1 || dto.getOrderState() == 7 
					|| dto.getOrderState() == 9 ) && dto.getDetailState() == 0) {
	...
	public void updateOrderDetailState(Map<String, Object> map) throws Exception {
		try {
			...
			if(detailState == 1) {
				// 구매 확정인 경우 적립금 추가
				long orderDetailId = Long.parseLong((String)map.get("orderDetailId"));
				Long memberId = (Long)map.get("memberId");
				
				ProductOrder order = mapper.findOrderByOrderDetail(orderDetailId);
				
				Point up = new Point();
				up.setMemberId(memberId);
				up.setOrderId(order.getOrderId());
				up.setPoints(order.getSavedPoint());
				up.setClassify(1); // 1:적립, 2:사용, 3:소멸, 4:주문취소/판매취소
				up.setBaseDate(order.getOrderDate());
				up.setReason("구매확정");
				mapper.insertUserPoint(up);
			}
	...
	public Payment stateView(Map<String, Object> map) {
		...
		try {
			...
			if((dto.getOrderState() == 1 || dto.getOrderState() == 7 
					|| dto.getOrderState() == 9 ) && dto.getDetailState() == 0) {
				dto.setOrderStateInfo("상품 준비중");
			} else {
				System.out.println(OrderState.ORDERSTATEINFO[dto.getOrderState()]);
				dto.setOrderStateInfo(OrderState.ORDERSTATEINFO[dto.getOrderState()]);;
			}
		} catch (Exception e) {
