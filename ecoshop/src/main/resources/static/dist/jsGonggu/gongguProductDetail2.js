...
function printReview(data) {
    ...
    if (dataCount > 0) {
        reviewsHtml = list.map(item => {
            const { gongguOrderDetailId, name, rate, content, regDate, answer, answerDate, listReviewImg, reviewHelpfulCount, userReviewHelpful } = item;
...
function reviewWrite(){
	...
	const fn = function(data) {
		const { didIBuyProduct } = data;
		
		let reviewsHtml = ''; // 전체 리뷰 목록을 담을 변수

		if (didIBuyProduct.length !== 0) {
			console.log(didIBuyProduct);
			reviewsHtml = didIBuyProduct.map(item => {
...
function printInquiry(data) {
    ...
    if (dataCount > 0) {
		...
		out += list.map(item => {
            ...
            const { gongguInquiryId, name, title, content, regDate, answer, answerDate, secret, deletePermit } = item;
            // 답변 상태를 설정
            const answerState = answerDate ? `<td class="inquiryStatus" data-gongguInquiryId="${gongguInquiryId}" width="150px">답변완료</td>` : `<td class="inquiryStatus" data-gongguInquiryId="${gongguInquiryId}" width="150px">답변대기</td>`;
			...
			const inquirySimple = answerState + `<td class="inquiryTitle ${isSecretTitle}" data-gongguInquiryId="${gongguInquiryId}">${title}</td>
									<td class="inquiryName" data-gongguInquiryId="${gongguInquiryId}" width="150px">${name}</td>
									<td class="inquiryDate" data-gongguInquiryId="${gongguInquiryId}" width="230px">${regDate}</td>`;
			...
			const inquiryDetail = `<tr class="inquiryDetailTr d-none">
										<td colspan="4">
											<div class="">
												<div id="inquiryDetail${gongguInquiryId}" class="inquiry-detailInfo">
            ...
            return `
				<tr class="inquiryTr ${isSecretInquiry} ${isMyInquiry}" data-gongguInquiryId="${gongguInquiryId}">
					${inquirySimple}
				</tr>
				${inquiryDetail}
            `;
...
$(function(){
	$('div.detailTabList').on('click', 'p.myInquiryDelete', function(){
		...
		let gongguInquiryId = $tr.attr('data-gongguInquiryId');
		
		console.log(gongguInquiryId);
		...
		const contextPath = document.getElementById('web-contextPath').value;
		const url = contextPath + '/gongguInquiry/delete?gongguInquiryId=' + gongguInquiryId;
...
$(function(){
	$('div.detailTabList').on('click', 'td.inquiryTitle', function(){
		...
		let gongguInquiryId = $(this).attr('data-gongguInquiryId');
