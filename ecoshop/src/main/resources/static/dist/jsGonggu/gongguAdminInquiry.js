...
$(function(){
    $('div#content-area').on('click', 'button.addAnswer', function(){
        ...
        $(f).find('input[name="gongguInquiryId"], input[name="answerId"]').remove();

        let inputTag1 = document.createElement("input");
        inputTag1.setAttribute('type', 'hidden');
        inputTag1.setAttribute('name', 'gongguInquiryId');
        inputTag1.value = $(this).attr('data-gongguInquiryId');
        f.appendChild(inputTag1);

        let inputTag2 = document.createElement("input");
        inputTag2.setAttribute('type', 'hidden');
        inputTag2.setAttribute('name', 'answerId');
        inputTag2.value = $(this).attr('data-managerId');
        f.appendChild(inputTag2);
...
$(function(){
	$('div#content-area').on('click', 'button.updateAnswer', function(){
		...
		// name 넣어주기
		const gongguInquiryId = $(this).attr('data-gongguInquiryId');
		...
		inputTag1.setAttribute('type', 'hidden');
		inputTag1.setAttribute('name', 'gongguInquiryId');
		inputTag1.value = gongguInquiryId;
...
$(function(){
	...
	$('div#content-area').on('click', 'button.removeAnswer', function(){
		...
		location.href = 'deleteAnswer?gongguInquiryId=' + gongguInquiryId;
	});
})
...
$(function(){
	...
	$('div#content-area').on('click', 'button.deleteInquiry', function(){
		...
		location.href = 'deleteInquiry?deleteInquiry?gongguInquiryId=' + gongguInquiryId;
	});
});
...
function listInquiry(page) {
	...
	let gongguProductName = '';
	let kwd = '';
	if(searchType === 'productName' && searchValue !== ''){
		gongguProductName = document.getElementById('searchValue').value;
	} else if(searchType === 'kwd') {
		kwd = document.getElementById('searchValue').value;
	}
	
	let url = 'inquiryList';
	let params = {gongguProductName:gongguProductName, kwd:kwd, pageNo:page};
